...
const BanhTVService = require('../services/BanhTVService');

router.post('', function (req, res, next) {
  const itemUrl = Buffer.from(req.body.url, 'base64').toString('utf-8');
  const poster = Buffer.from(req.body.poster, 'base64').toString('utf-8');
  const hash = crypto.createHash('md5').update(itemUrl).digest("hex");
  ...
  Item.findOne({hash: hash}, function (err, item) {
    if (err) {
      ...
      const parseUrl = url.parse(itemUrl);
      if (parseUrl.hostname === 'vung.tv') {
        let itemPath = itemUrl.split('vung.tv')[1];
      ...
      } else if (parseUrl.hostname === 'banhtv.com') {
        console.log('parsing banhtv film');
        console.log(itemUrl);
        BanhTVService.crawItem(itemUrl).then((crawItem) => {
        ...
        res.status(500)
        res.send({err: `unsupported movie provider ${url.hostname}`})
      }
    }
  });
});
